...
# Separate function to handle reverse operation
def handle_reverse_command(text: str) -> str:
    # e.g. input: "reverse Soham"
    parts = text.split(maxsplit=1)
    if len(parts) < 2:
        return "Please provide a word to reverse, e.g. 'reverse Soham'"
    word_to_reverse = parts[1]
...
def process(query: Query):
    ...
    # Handle reverse command separately
    if text.lower().startswith("reverse "):
        reversed_result = handle_reverse_command(text)
        logger.info("Reverse operation result: %s", reversed_result)
        return {"input": text, "result": reversed_result}

    # For other queries, use LLM with date awareness
    now = datetime.now()
